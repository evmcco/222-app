...
test('Saturday, Sunday and Labor Day Monday stay in 2026 Week 1', () => {
  for (const date of ['2026-09-05T18:00Z', '2026-09-06T18:00Z', '2026-09-07T23:30Z', '2026-09-08T06:59:59Z']) {
    assert.deepEqual(resolveCfbWeek(calendar, new Date(date)), { season_year: 2026, season_type: 2, week_number: 1 });
  }
});

test('cached calendar advances at the exact Week 2 boundary', () => {
  assert.deepEqual(resolveCfbWeek(calendar, new Date('2026-09-08T07:00Z')), { season_year: 2026, season_type: 2, week_number: 2 });
});

test('uses supplied season year instead of the device calendar year', () => {
  assert.deepEqual(resolveCfbWeek({ season: { year: 2026 }, week: { number: 1 } }, new Date('2027-01-05')), { season_year: 2026, season_type: 2, week_number: 1 });
});
...
test('validates metadata and accepts Week 0', () => {
  ...
  assert.deepEqual(resolveCfbWeek({ season: { year: 2026 }, week: { number: 0 } }), { season_year: 2026, season_type: 2, week_number: 0 });
});

test('realtime membership requires both season and week', () => {
  const week = { season_year: 2026, season_type: 2, week_number: 1 };
...
test('separates ESPN combined Week 1 into Week 0 and Week 1', () => {
  ...
  const game = { season_year: 2026, season_type: 2, week_number: 1, game_date: '2026-08-29T16:00:00' };
  assert.equal(matchesSelectedWeek(game, { season_year: 2026, season_type: 2, week_number: 0 }, true), true);
  assert.equal(matchesSelectedWeek(game, { season_year: 2026, season_type: 2, week_number: 1 }, true), false);
...
test('postseason Week 1 cannot match regular-season Week 1', () => {
  const regular = { season_year: 2026, season_type: 2, week_number: 1 };
  assert.equal(isInCfbWeek({ ...regular, season_type: 3 }, regular), false);
});

test('cached calendar crosses into postseason without needing ESPN metadata refresh', () => {
  const extended = structuredClone(calendar);
  extended.leagues[0].calendar.push({ value: '3', entries: [
    { value: '1', startDate: '2026-12-14T08:00Z', endDate: '2027-01-28T07:59Z' },
  ] });
  assert.deepEqual(resolveCfbWeek(extended, new Date('2027-01-01')), {
    season_year: 2026, season_type: 3, week_number: 1,
  });
});
